...
MQTT_BROKER = "192.168.188.48"
MQTT_NODE = "bounding_box"
...
# Envía las coordenadas del bounding box a través de MQTT
def send_coordinates(xmin, ymin, xmax, ymax):
    payload = f'{xmin},{ymin},{xmax},{ymax}'
    mqtt_client.publish(MQTT_NODE, payload)
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            # Envía las coordenadas del bounding box a través de MQTT
            send_coordinates(xmin, ymin, xmax, ymax)
